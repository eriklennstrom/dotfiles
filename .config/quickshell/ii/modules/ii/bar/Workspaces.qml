...
Item {
    ...
    WheelHandler {
        onWheel: (event) => {
            const groupSize = root.workspacesShown;
            const groupStart = Math.floor((root.effectiveActiveWorkspaceId - 1) / groupSize) * groupSize + 1;
            const groupEnd = groupStart + groupSize - 1;
            if (event.angleDelta.y < 0 && root.effectiveActiveWorkspaceId < groupEnd)
                Hyprland.dispatch(`workspace r+1`);
            else if (event.angleDelta.y > 0 && root.effectiveActiveWorkspaceId > groupStart)
                Hyprland.dispatch(`workspace r-1`);
        }
